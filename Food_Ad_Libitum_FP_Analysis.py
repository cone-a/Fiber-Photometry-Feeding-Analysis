# -*- coding: utf-8 -*-
"""
Created on Mon Jun  3 13:11:58 2024
...
directory = 'D:/Fiber Photometry Food Restrict/Control_Food_Restrict/'
...
# Process all animals and store results in food_ad_lib_470
food_ad_lib_470 = {}
...
for mouse in dfs_gcamp:
        events = contvar_peh(var_ts = dfs_gcamp[mouse]['Time'], 
                              ...
                              bin_width = False)
        # break
        food_ad_lib_470[mouse]=events ## saves array for each peri-event for all animals
...
to_line_points = chain.from_iterable(food_ad_lib_470.values())#### TImestamped ambush events

lined_up_points = np.array(list(to_line_points))

# Formatted so can process SEM and heatmap
lined_up_points = np.mean(list(food_ad_lib_470.values()), axis=1)
...
ax.set_title('Aggregrated 470 Signal Fed')
ax.margins(x=0)        
...
for mouse in dfs_gcamp:
        events = contvar_peh(var_ts = dfs_iso[mouse]['Time'], 
                             ...
                             bin_width = False)
        # break
        food_ad_lib_415[mouse]=events ## saves array for each peri-event for all animals
...
# Formatted so can process SEM and heatmap
to_line_points = chain.from_iterable(food_ad_lib_415.values())#### TImestamped ambush events

lined_up_points = np.array(list(to_line_points))

# Formatted so can process SEM and heatmap
lined_up_points = np.mean(list(food_ad_lib_415.values()), axis=1)
...
ax.set_xlabel('Time (sec)')
ax.set_ylabel('Z-Score')
ax.set_title('Aggregrated 415 Signal Fed')
ax.margins(x=0)       
